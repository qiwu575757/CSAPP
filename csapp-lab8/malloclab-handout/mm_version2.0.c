/*
 * mm_version2.0 显示空闲链表+ 地址顺序排列+ 首次适配
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
/* 按照 地址顺序 维护空闲链表，链表中每个块的地址小于它后继块的地址 */
static void add_to_empty_list(void *bp) {
    for (void *ptr = root; ptr != NULL; ptr = SUCC_BLKP(ptr)) {
            /* bp > ptr is certain */
            /* 若ptr大于当前节点且小于其后继节点 */
            if (SUCC_BLKP(ptr) && bp < SUCC_BLKP(ptr)) {
                /* bp->pred = ptr */
                PUT(PRED(bp), ptr);
                /* bp->succ = address(ptr->succ) */
                PUT(SUCC(bp), SUCC_BLKP(ptr));
                /* (bp->succ)->pred = bp*/
                if (SUCC_BLKP(ptr) != NULL)
                    PUT(PRED(SUCC_BLKP(ptr)), bp);
                /* root->succ = bp */
                PUT(SUCC(ptr), bp);
                break;
            } 
            /* 当前节点后继为空，说明已经将空闲链表遍历完，可以直接插入*/
            else if (SUCC_BLKP(ptr) == NULL) {
                /* bp->pred = ptr */
                PUT(PRED(bp), ptr);
                /* bp->succ = address(ptr->succ) */
                PUT(SUCC(bp), SUCC_BLKP(ptr));
